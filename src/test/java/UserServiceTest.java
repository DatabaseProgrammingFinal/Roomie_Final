...
public class UserServiceTest {
   ...
   public void testUpdateUser() {
       User user = new User("user00", "password", "SenderUser",
                            "Dorm A", "101", "http://example.com/profile.jpg", 10);
    ...
    public void testDeleteUser() {
        boolean result = userService.deleteUser("user02");
        assertTrue(result); // true면 성공
    }

    @Test
    public void testFindUserByLoginId() {
        User user = userService.findUserByLoginId("user01");
        assertNotNull(user); // user가 null이 아니면 성공
        assertEquals("user01", user.getLoginId());
    }
