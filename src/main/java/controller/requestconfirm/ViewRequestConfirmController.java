...
public class ViewRequestConfirmController implements Controller {
    private RequestConfirmService requestConfirmService;

    public ViewRequestConfirmController() {
        this.requestConfirmService = new RequestConfirmService();
    }

    @Override
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        int requestConfirmId = Integer.parseInt(request.getParameter("requestConfirmId")); // requestConfirm ID로 조회

        try {
            // 디버깅: ProvideConfirm ID 확인
            System.out.println("Debug - requestConfirm ID: " + requestConfirmId);
            ...
            return "/requestConfirm/rentalStatusView.jsp"; // JSP로 이동
        } catch (Exception e) {
            e.printStackTrace();
