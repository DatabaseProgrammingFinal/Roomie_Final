...
public class UserDAO {
	...
	public int create(User user) throws SQLException {
		...
		Object[] param = new Object[] {user.getUserId(), user.getPassword(), 
						user.getName(), user.getDorm(), user.getRoom(), 
	...
	public int update(User user) throws SQLException {
		String sql = "UPDATE USERINFO "
					+ "SET password=?, name=?, dorm=?, room=?, commId=? "
					+ "WHERE userid=?";
		Object[] param = new Object[] {user.getPassword(), user.getName(), 
					user.getDorm(), user.getRoom(), 
					(user.getCommId()!=0) ? user.getCommId() : null, 
					user.getUserId()};				
	...
	public User findUser(String userId) throws SQLException {
        String sql = "SELECT password, name, dorm, room, commId, cName "
	...
	public List<User> findUserList() throws SQLException {
        String sql = "SELECT userId, name, dorm, NVL(commId,0) AS commId, cName " 
        		   + "FROM USERINFO u LEFT OUTER JOIN Community c ON u.commId = c.cId "
        		   + "ORDER BY userId";
	...
	public List<User> findUsersInCommunity(int communityId) throws SQLException {
        String sql = "SELECT userId, name, dorm, room FROM UserInfo "
     				+ "WHERE commId = ?";                         
